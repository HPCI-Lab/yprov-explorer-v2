...
function App() {
  ...
  useEffect(() => {
    ...
    const syncNodeWithHistory = (event) => {
      const nodeId =
        event?.state?.idNodo || window.location.hash.substring(1); // Get the node ID from the URL
    ...
    const syncInitialNode = () => {
      ...
      if (initialNodeId && graphData) {
        ...
        if (initialNodeDetails) {
          ...
          window.history.replaceState({ idNodo: initialNodeId }, "", `#${initialNodeId}`);
        }
      }
    };
  ...
  const findNodeDetails = (nodeId, graphData) => {
    ...
    const wasGeneratedBy =
      links
        .filter((link) => link.type === "wasGeneratedBy" && link.source?.id === nodeId)
        .map((link) => link.target.id)
        .join(", ") || "None";

    const used =
      links
        .filter((link) => link.type === "used" && link.source?.id === nodeId)
        .map((link) => link.target.id)
        .join(", ") || "None";

    const wasDerivedFrom =
      links
        .filter((link) => link.type === "wasDerivedFrom" && link.source?.id === nodeId)
        .map((link) => link.target.id)
        .join(", ") || "None";

    const wasInformedBy =
      links
        .filter((link) => link.type === "wasInformedBy" && link.source?.id === nodeId)
        .map((link) => link.target.id)
        .join(", ") || "None";

    const wasAssociatedWith =
      links
        .filter((link) => link.type === "wasAssociatedWith" && link.source?.id === nodeId)
        .map((link) => link.target.id)
        .join(", ") || "None";

    const wasStartedBy =
      links
        .filter((link) => link.type === "wasStartedBy" && link.source?.id === nodeId)
        .map((link) => link.target.id)
        .join(", ") || "None";

    const hadMember =
      links
        .filter((link) => link.type === "hadMember" && link.source?.id === nodeId)
        .map((link) => link.target.id)
        .join(", ") || "None";

    const wasAttributedTo =
      links
        .filter((link) => link.type === "wasAttributedTo" && link.source?.id === nodeId)
        .map((link) => link.target.id)
        .join(", ") || "None";
    ...
    const generated =
      links
        .filter((link) => link.type === "wasGeneratedBy" && link.target?.id === nodeId)
        .map((link) => link.source.id)
        .join(", ") || "None";

    const wasUsedBy =
      links
        .filter((link) => link.type === "used" && link.target?.id === nodeId)
        .map((link) => link.source.id)
        .join(", ") || "None";

    const derives =
      links
        .filter((link) => link.type === "wasDerivedFrom" && link.target?.id === nodeId)
        .map((link) => link.source.id)
        .join(", ") || "None";
          ...
          element={
            <Flex direction="column" h="100vh" w="100vw" bg="black" color="white">
              ...
              <Flex flex="1" position="relative" bg="black">
                <Sidebar onOpenPanel={onOpenPanel} homeRoute={"/catalog"} />
