...
const MultiLevel = ({ mainGraphData, mainFileName, setGraphData, updateJsonLabel }) => {
  // --- State ---
  ...
  return (
    <Box className="ml-panel">
      ...
      {showAdvanced && (
        <>
          ...
          <Box className="ml-section">
            ...
            <label className="ml-tooltip-wrapper">
      ...
      {levelCount > 0 && (
        <Box className="ml-section">
          <h4 className="ml-subtitle">Select Level</h4>
          <select className="ml-select" value={selectedLevel ?? ""} onChange={(e) => handleLevelSelect(Number(e.target.value))}>
