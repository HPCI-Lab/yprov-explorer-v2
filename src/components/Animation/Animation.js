...
const Animation = ({ activities = [], linksByActivityRef, onIndexChange }) => {
  ...
  // Determine which activities overlap current subset
  const subsetIndices = activities
    .map((a, i) => ({
      i,
      overlaps: (a.start ?? 0) < endTime && (a.end ?? (a.start ?? 0)) > startTime
    }))
    .filter(x => x.overlaps)
    .map(x => x.i);

  const startIdx = subsetIndices.length ? subsetIndices[0] : 0;
  const endIdx = subsetIndices.length ? subsetIndices[subsetIndices.length - 1] : activities.length - 1;
  ...
  const applyOpacities = useCallback((effectiveTime, subsetTime) => {
    ...
    allNodesRef.current?.style("opacity", 0.15);
    allLinksRef.current?.style("opacity", 0.15);
    ...
    activities.forEach((act) => {
      ...
      if (!nodeSel.empty()) {
        nodeSel.style("opacity", actOpacity);
      }
      ...
      linked.forEach((l) => {
        ...
        const isActivity = activities.some(a => a.id === l.entId);
  ...
  }, [activities, linksByActivityRef, getActivityOpacity]);
  ...
  useEffect(() => {
    ...
    if (typeof onIndexChange === "function") {
      onIndexChange({ currentIndex, currentTime, subset: [startTime, endTime] });
    }
  }, [subsetTime, currentIndex, currentTime, useTimestamps, activities, applyOpacities, onIndexChange, startIdx, endIdx, startTime, endTime, minTime]);
